...
if __name__ == "__main__":
    ...
    l_arg_count_dataset = {'data': []}
    l_arg_count_lex_dataset = {'data': []}
    l_arg_count_tok_dataset = {'data': []}
    l_arg_count_tok_dir_dataset = {'data': []}
                                ...
                                if node2['nodeID'] == edge['toID'] and node2['type'] == 'I':
                                                    ...
                                                    for node4 in content['nodes']:
                                                        if id[0] == node4['nodeID'] and node4['type'] == 'I':
                                                            if id[2] == 'from_prop':
                                                                ...
                                                                tok_count_dir_text += ' [IFP] ' # format: [premise prop] [inference: first was premise] 
                                                            elif id[2] == 'to_prop':
                                                                ...
                                                                tok_count_dir_text += ' [IFC] ' # format: [conclusion prop] [inference: first was conclusion]
                                                            tok_count_text += ' [INF] ' 
                                                            text += '. ' + node4['text']
                                                elif node3['type'] == 'CA':
                                                    for node4 in content['nodes']:
                                                        if id[0] == node4['nodeID'] and node4['type'] == 'I':
                                                            if id[2] == 'from_prop':
                                                                ...
                                                                tok_count_dir_text += ' [CNP] ' # format: [premise/attacker prop] [conclict: first was premise/attacker] 
                                                            elif id[2] == 'to_prop':
                                                                ...
                                                                tok_count_dir_text += ' [CNC] ' # format: [conclusion/target prop] [conclict: first was conclusion/target] [premise/attacker prop]
                                    ...
                                    if l_text != '' and text != '':
                                        if is_hypothesis:
                                            ...
                                            ##NEW##
                                            l_arg_count_tok_dataset['data'].append([l_text + '. ' + tok_count_text, 0])
                                            l_arg_count_tok_dir_dataset['data'].append([l_text + '. ' + tok_count_dir_text, 0])
                                        else:
                                            ...
                                            ##NEW##
                                            l_arg_count_tok_dataset['data'].append([l_text + '. ' + tok_count_text, 1])
                                            l_arg_count_tok_dir_dataset['data'].append([l_text + '. ' + tok_count_dir_text, 1])
    ...
    with open('data_ready/l_arg_count_hypothesis.json', "w") as outfile:
        json.dump(l_arg_count_dataset, outfile, indent=4)
    
    with open('data_ready/l_arg_count_lex_hypothesis.json', "w") as outfile:
        json.dump(l_arg_count_lex_dataset, outfile, indent=4)
    
    with open('data_ready/l_arg_count_tok_hypothesis.json', "w") as outfile:
        json.dump(l_arg_count_tok_dataset, outfile, indent=4)
    
    with open('data_ready/l_arg_count_tok_dir_hypothesis.json', "w") as outfile:
        json.dump(l_arg_count_tok_dir_dataset, outfile, indent=4)